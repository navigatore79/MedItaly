...
function therapyRows(meds,intakes,day){
  ...
  return (meds||[]).filter(m=>m.active).map(m=>{
    const times=(m.medication_schedules||[]).filter(sc=>sc.time_of_day);
    return `<div class="item therapy-item"><div><b>${esc(m.name)} ${esc(m.dose||'')}</b><div class="small muted">${esc(m.route||'')} · Attiva</div><div class="small">${times.map(sc=>`${esc(sc.time_of_day?.slice(0,5)||'—')} · ${esc(intakeState(recorded.get(sc.id)))}`).join('<br>')||'Nessun orario'}</div></div><div class="row therapy-actions"><button type="button" class="btn secondary edit-med" data-med="${esc(m.id)}">Modifica</button><button type="button" class="btn danger remove-med" data-med="${esc(m.id)}">Rimuovi</button></div></div>`;
  }).join('')||'<p class="muted">Nessuna terapia attiva.</p>';
}
...
function loadTherapyPatientList(){
  const box=$('therapyPatientList');if(!box)return;
  box.innerHTML=patients.map(p=>`<button type="button" class="patient therapy-patient" data-id="${esc(p.patient_id)}"><b>${esc(p.profile?.full_name||'Paziente')}</b><span>Apri terapie →</span></button>`).join('')||'<p class="notice">Nessun paziente assegnato. Apri Amministrazione per associare un paziente al medico.</p>';
  box.querySelectorAll('.therapy-patient').forEach(button=>button.onclick=async()=>{
    document.querySelector('.nav button[data-view="patients"]').click();await openPatient(button.dataset.id);
    $('patientDetail').querySelector('[data-tab="therapy"]')?.click();$('patientDetail').scrollIntoView({behavior:'smooth',block:'start'});
  });
}
...
async function loadAdmin(){
  ...
  <div class="admin-columns"><section class="admin-panel" aria-labelledby="adminAssociationTitle"><div class="admin-panel-head"><div><h2 id="adminAssociationTitle">Associa paziente e medico</h2><p>Il medico selezionato potrà consultare il percorso del paziente.</p></div></div><div class="admin-association-form"><label class="field" for="adminLinkPatient">Paziente<select id="adminLinkPatient"><option value="">Seleziona il paziente</option>${activePatients.map(p=>`<option value="${esc(p.id)}">${esc(p.full_name||'Paziente')}${p.role==='Administrator'?' · Area personale':''}</option>`).join('')}</select></label><span class="admin-form-arrow" aria-hidden="true">→</span><label class="field" for="adminLinkDoctor">Medico<select id="adminLinkDoctor"><option value="">Seleziona il medico</option>${activeDoctors.map(p=>`<option value="${esc(p.id)}">${esc(p.full_name||'Medico')}</option>`).join('')}</select></label></div><button id="adminAddLink" class="btn admin-primary" type="button" ${!activePatients.length||!activeDoctors.length?'disabled':''}>Associa paziente al medico</button><div id="adminLinkResult" role="status" aria-live="polite"></div><p class="admin-access-note">Una richiesta in attesa per la stessa coppia viene segnata come accettata. Il contatto Test Medico segue un percorso separato.</p></section>
...
async function openPatient(id){
  ...
  const paintIntakes=()=>{
    ...
    $('therapyCurrent').innerHTML=therapyRows(meds.data,patientIntakes,romeDay());bindTherapyActions();
  ...
  const resetMedForm=()=>{editingMedication=null;$('medFormTitle').textContent='Nuova terapia';$('saveMed').textContent='Salva terapia';$('cancelMedEdit').classList.add('hidden');['medName','medDose','medInstr','medStart','medEnd','medTimes'].forEach(key=>$(key).value='');$('medRoute').value='Orale';$('medMsg').innerHTML='';};
  $('cancelMedEdit').onclick=resetMedForm;
  function bindTherapyActions(){
    $('therapyCurrent').querySelectorAll('.edit-med').forEach(button=>button.onclick=()=>{
      const m=(meds.data||[]).find(x=>x.id===button.dataset.med&&x.active);if(!m)return;
      editingMedication=m.id;$('medFormTitle').textContent='Modifica terapia · '+m.name;$('saveMed').textContent='Conferma modifica';$('cancelMedEdit').classList.remove('hidden');
      $('medName').value=m.name;$('medDose').value=m.dose||'';$('medRoute').value=m.route||'';$('medInstr').value=m.instructions||'';$('medStart').value=m.starts_on||'';$('medEnd').value=m.ends_on||'';
      $('medTimes').value=(m.medication_schedules||[]).filter(sc=>sc.time_of_day).map(sc=>sc.time_of_day.slice(0,5)).join(',');
      $('medMsg').innerHTML='';$('medFormTitle').scrollIntoView({behavior:'smooth',block:'center'});
    });
    $('therapyCurrent').querySelectorAll('.remove-med').forEach(button=>button.onclick=async()=>{
      const m=(meds.data||[]).find(x=>x.id===button.dataset.med&&x.active);if(!m)return;
      if(!confirm(`Rimuovere ${m.name} dal piano di ${p?.full_name||'questo paziente'} e inviare una notifica?`))return;
      button.disabled=true;const {data:outboxId,error}=await sb.rpc('clinician_remove_care_item',{p_patient:id,p_kind:'medication',p_item:m.id});
      if(error){button.disabled=false;return out($('medMsg'),'Rimozione non riuscita: '+error.message);}
      const pushed=await dispatch(outboxId);await openPatient(id);$('patientDetail').querySelector('[data-tab="therapy"]')?.click();
      out($('medMsg'),`Farmaco rimosso dal piano. Notifica in-app registrata${pushed?' e push inviata':' · push in coda o consegna da verificare'}.`,true);
    });
  }
  $('saveMed').onclick=async()=>{
    ...
    if($('medStart').value&&$('medEnd').value&&$('medEnd').value<$('medStart').value)return out($('medMsg'),'La data di fine precede l’inizio.');
    if(editingMedication){
      const button=$('saveMed');button.disabled=true;
      const {data:outboxId,error}=await sb.rpc('clinician_update_medication',{p_patient:id,p_item:editingMedication,p_name:name,p_dose:$('medDose').value.trim(),p_route:$('medRoute').value.trim(),p_instructions:$('medInstr').value.trim(),p_starts_on:$('medStart').value||null,p_ends_on:$('medEnd').value||null,p_times:times});
      button.disabled=false;if(error)return out($('medMsg'),'Modifica non riuscita: '+error.message);
      const pushed=await dispatch(outboxId);await openPatient(id);$('patientDetail').querySelector('[data-tab="therapy"]')?.click();out($('medMsg'),`Terapia aggiornata. Notifica in-app registrata${pushed?' e push inviata':' · push in coda o consegna da verificare'}.`,true);return;
    }
    const uid=(await sb.auth.getUser()).data.user.id;
