alter table public.patient_clinician_requests
  ...
  add column if not exists applied_protocol_id uuid references public.monitoring_protocols(id),
  add column if not exists protocol_assignment_id uuid references public.patient_protocol_assignments(id);
...
begin
  ...
  if nullif(btrim(p_procedure_label),'') is null or p_performed_on is null
     or p_performed_on > current_date or p_performed_on < date '1900-01-01'
...
revoke all on function public.review_link_with_care(uuid,text,text,date,uuid,jsonb,jsonb,text) from public, anon;
grant execute on function public.review_link_with_care(uuid,text,text,date,uuid,jsonb,jsonb,text) to authenticated;
