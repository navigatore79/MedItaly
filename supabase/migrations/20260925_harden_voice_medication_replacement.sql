...
begin
  ...
  where id=p_old_medication and patient_id=p_patient and active for update;
  if not found then raise exception 'ACTIVE_MEDICATION_NOT_FOUND'; end if;
  if length(trim(coalesce(p_new_name,'')))<2 or length(trim(coalesce(p_new_name,'')))>200
     or length(trim(coalesce(p_dose,'')))<1 or length(trim(coalesce(p_dose,'')))>100
     or length(trim(coalesce(p_route,'')))<2
     or p_starts_on is null or p_ends_on is not null and p_ends_on<p_starts_on
     or p_times is null or cardinality(p_times) not between 1 and 6 then
