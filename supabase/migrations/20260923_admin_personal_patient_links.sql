-- Ammette l’area personale paziente dell’amministratore come destinatario di un medico distinto.
...
begin
  ...
  if p_patient = p_clinician then raise exception 'SELF_LINK_FORBIDDEN'; end if;
  if not exists (
    select 1 from public.profiles p where p.id=p_patient
      and p.role::text in ('Patient','Administrator') and p.account_active
